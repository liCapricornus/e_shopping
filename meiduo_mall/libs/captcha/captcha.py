...
class Bezier:
    def __init__(self):
        # self.tsequence = tuple([t / 20.0 for t in range(21)])  # 20220531
...
class Captcha(object):
    ...
    def noise(self, image, number=50, level=2, color=None):
        ...
        # dx = width / 10
        dx = width // 10     # 20220531
        width -= dx
        # dy = height / 10
        dy = height // 10    # 20220531
        height -= dy
        draw = Draw(image)
    ...
    def text(self, image, fonts, font_sizes=None, drawings=None, squeeze_factor=0.75, color=None):
        ...
        offset = int((width - sum(int(i.size[0] * squeeze_factor)
                      ...
                      char_images[-1].size[0]) // 2)   # 20220531
        for char_image in char_images:
            c_width, c_height = char_image.size
            # mask = char_image.convert('L').point(lambda i: i * 1.97)
            mask = char_image.convert('L').point(lambda i: i * 2)   # 问题解决！注意错误提示的代码，debug先看错误提示+代码
            image.paste(char_image,
                        # (offset, int((height - c_height) / 2)),
                        (offset, int((height - c_height) // 2)),   # 20220531
                        mask)
